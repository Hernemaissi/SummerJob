...
class Rfp < ActiveRecord::Base
 ...
 #Returns true if the sender and target need to make a contract to finish round 2
  ...
  def self.can_send?(sender_user, target)
    sender_user.company && Rfp.valid_target?(sender_user.company, target) &&  !sender_user.company.has_sent_rfp?(target) && (sender_user.company.values_decided? && target.values_decided?) && !Game.get_game.in_round(1)
  end
